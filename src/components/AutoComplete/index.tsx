...
const Main = ({ apiFunction, setBooking, booking, appliedOn }) => {
  ...
  const fetchSuggestions = useCallback(
    debounce(async (inputValue: any) => {
      ...
      try {
        if (inputValue) {
          const response: any = await apiFunction(countryCode, inputValue);
  ...
  useEffect(() => {
    fetchSuggestions(booking?.destination_pincode);
  }, [booking?.destination_pincode, fetchSuggestions]);

  //   useEffect(() => {
  //     setInput(booking?.destination_pincode || "");
  ...
  return (
    <>
      <FormInput
        ...
        placeholder="Select Destination Zipcode"
        value={booking?.destination_pincode}
      />
      {showSuggestions && booking?.destination_pincode && (
